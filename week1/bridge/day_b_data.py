...
# ---------- 1. 리스트 기본 ----------
n = len(quotes)                    # TODO: quotes 의 건수
first = quotes[0]                 # TODO: 첫 번째 항목 (딕셔너리)
last = quotes[-1]                  # TODO: 마지막 항목. 힌트: 인덱스 -1
...
# ---------- 2. 딕셔너리 꺼내기 ----------
second_author = quotes[1]["author"]         # TODO: 두 번째 항목의 author
second_tags = quotes[1]["tags"].split(",")           # TODO: 두 번째 항목의 tags 를 쉼표로 쪼갠 리스트. 힌트: .split(",")
...
# ---------- 3. 컴프리헨션으로 한 열만 뽑기 ----------
authors = [q["author"] for q in quotes]               # TODO: author 만 모은 리스트 (길이 5)

assert authors == ["Albert Einstein", "J.K. Rowling", "Albert Einstein", "Jane Austen", "Albert Einstein"]

for q in quotes:
    assert q["author"] in authors
# ---------- 4. 조건으로 거르기 ----------
einstein_only = [q for q in quotes if q["author"] == "Albert Einstein"]         # TODO: author 가 "Albert Einstein" 인 항목만 (딕셔너리 리스트, 길이 3)
...
# ---------- 5. 세기 ----------
counts = Counter(q["author"] for q in quotes)               # TODO: Counter 로 저자별 건수
top1 = counts.most_common(1)[0]                  # TODO: counts.most_common(1)[0]  → ("Albert Einstein", 3)
...
quotes.append({"author": "Mark Twain", "tags": "humor"})
...
first_tags_by_author = {}  # TODO``
for q in quotes:
    if q["author"] not in first_tags_by_author:
        first_tags_by_author[q["author"]] = q["tags"]
...
tag_counts = Counter(t for q in quotes for t in q["tags"].split(","))          # TODO
